...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		$total = 0;
		$totalprice = 0;
		//TODO 产品价格 成本价格 由 ifnull(o.marketprice, g.marketprice) as marketprice,g.productprice 换成在规格中选择
		$sql = 'SELECT f.id,f.total,f.goodsid,g.total as stock, o.stock as optionstock, g.maxbuy,g.title,g.thumb,
				o.marketprice,o.productprice,o.title as optiontitle,f.optionid,o.specs FROM ' 
				. tablename('ewei_shop_member_cart') . ' f ' . ' left join ' . tablename('ewei_shop_goods') . ' g on f.goodsid = g.id ' 
				. ' left join ' . tablename('ewei_shop_goods_option') . ' o on f.optionid = o.id ' . ' where 1 ' . $condition . ' ORDER BY `id` DESC ';
		...
		//TODO kim 结算页面过来的 怎么会是POST的？ && $_W['ispost']
		...
		show_json(1, array('optionid' => $optionid, 'optiontitle' => $option['title'],'productprice'=>$option['productprice'],'marketprice'=>$option['marketprice']));
