...
if (!class_exists('CommissionModel')) {
	class CommissionModel extends PluginModel {
		public function calculate($orderid = 0, $update = true) {
			...
			$goods = pdo_fetchall("select og.id,og.realprice,og.total,g.hascommission,og.commission_sum, g.commission1_rate,g.commission1_pay,g.commission2_rate,g.commission2_pay,g.commission3_rate,g.commission3_pay from " . tablename('ewei_shop_order_goods') . '  og ' . ' left join ' . tablename('ewei_shop_goods') . ' g on g.id = og.goodsid' . ' where og.orderid=:orderid and og.uniacid=:uniacid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
			if ($set['level'] > 0) {
				//TODO 需判断是否启用管家功能，是则有1级佣金，否则不产生一级佣金	
				$openid=$_W['openid'];
				$member = m('member') -> getInfo($openid);
				// $isManager=($member['isagent'] == 1 && $member['status'] == 1);
				//TODO 得到上一级	需判断是否启用管家功能，是则有1级佣金，否则不产生一级佣金			
				$inviter=m('member')->getInfo($member['inviter']);
				$isManager=($inviter['isagent'] == 1 && $inviter['status'] == 1);
				//TODO 判断是否客服推荐，是则为拿一半佣金
				$cs=pdo_get("ewei_shop_order",array('id'=>$orderid,'uniacid'=>$_W['uniacid']),array('csopenid'));
				$isCs=empty($cs['csopenid']) ? 1:0.5;
				foreach ($goods as &$cinfo) {
					//TODO 原来的逻辑是由商品的实际售价来计算佣金，现在改为产品的分销金额来计算
					//$price = $cinfo['realprice'];		
					$price = $cinfo['commission_sum'];
					//TODO 说明：先判断产品是否有属于自己的分销规则。
					if ($cinfo['hascommission'] == 1) {
						...
						$cinfo['commission1'] = array('default' => (($set['level'] >= 1 && $isManager) || $isCs) ? round($set['commission1'] * $price / 100, 2)*$isCs . "" : 0);
						...
						foreach ($levels as $level) {
							$cinfo['commission1']['level' . $level['id']] = (($set['level'] >= 1 && $isManager) || $isCs)? round($level['commission1'] * $price / 100, 2)*$isCs . "" : 0;
		...
		public function getInfo($openid, $options = null) {
			...
			if ($level >= 1) {
				...
				if (in_array('total', $options)) {
					$level1_commissions = pdo_fetchall('select og.commission1,o.csopenid  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . " where (o.csopenid=:openid OR o.agentid=:agentid) and o.status>=1 and o.uniacid=:uniacid", array(':uniacid' => $_W['uniacid'],':openid'=>$member['openid'], ':agentid' => $member['id']));
					//var_dump($level1_commissions);exit;
				...
				if (in_array('ok', $options)) {
					$level1_commissions = pdo_fetchall('select og.commission1,o.csopenid  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . " where (o.csopenid=:openid OR o.agentid=:agentid) and o.status>=3 and ({$time} - o.createtime > {$day_times}) and og.status1=0  and o.uniacid=:uniacid", array(':uniacid' => $_W['uniacid'],':openid'=>$member['openid'], ':agentid' => $member['id']));
					foreach ($level1_commissions as $c) {
						$commission = iunserializer($c['commission1']);
				...
				if (in_array('lock', $options)) {
					$level1_commissions1 = pdo_fetchall('select og.commission1,o.csopenid  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . " where (o.csopenid=:openid OR o.agentid=:agentid) and o.status>=3 and ({$time} - o.createtime <= {$day_times})  and og.status1=0  and o.uniacid=:uniacid", array(':uniacid' => $_W['uniacid'],':openid'=>$member['openid'], ':agentid' => $member['id']));
				...
				if (in_array('apply', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,o.csopenid  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . " where (o.csopenid=:openid OR o.agentid=:agentid) and o.status>=3 and og.status1=1 and o.uniacid=:uniacid", array(':uniacid' => $_W['uniacid'],':openid'=>$member['openid'], ':agentid' => $member['id']));
				...
				if (in_array('check', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,o.csopenid  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . " where (o.csopenid=:openid OR o.agentid=:agentid) and o.status>=3 and og.status1=2 and o.uniacid=:uniacid ", array(':uniacid' => $_W['uniacid'],':openid'=>$member['openid'], ':agentid' => $member['id']));
					foreach ($level1_commissions2 as $c) {
						$commission = iunserializer($c['commission1']);
				...
				if (in_array('pay', $options)) {
					$level1_commissions2 = pdo_fetchall('select og.commission1,o.csopenid  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join  ' . tablename('ewei_shop_order') . ' o on o.id = og.orderid' . " where (o.csopenid=:openid OR o.agentid=:agentid) and o.status>=3 and og.status1=3 and o.uniacid=:uniacid ", array(':uniacid' => $_W['uniacid'],':openid'=>$member['openid'], ':agentid' => $member['id']));
					foreach ($level1_commissions2 as $c) {
						$commission = iunserializer($c['commission1']);
			...
			/*if ($level >= 3) {
				if ($level2 > 0) {
					...
					if (in_array('lock', $options)) {
						//TODO KIM {$time} - o.createtime <= {$day_times} 原来为:{$time} - o.createtime > {$day_times}
...
/*public function getInfos($openid, $options = null) {
			if (empty($options) || !is_array($options)) {
				$options = array();
			}
			global $_W;
...
if ($level >= 3) {
		...
		public function getCommission($goods) {
			...
			if ($goods['hascommission'] == 1) {
				//TODO $goods['marketprice'] =>$goods['commission_sum']
				$commission = $set['level'] >= 1 ? ($goods['commission1_rate'] > 0 ? ($goods['commission1_rate'] * $goods['commission_sum'] / 100) : $goods['commission1_pay']) : 0;
			} else {
				$openid = m('user') -> getOpenid();
				$level = $this -> getLevel($openid);
				if (!empty($level)) {
					$commission = $set['level'] >= 1 ? round($level['commission1'] * $goods['commission_sum'] / 100, 2) : 0;
				} else {
					$commission = $set['level'] >= 1 ? round($set['commission1'] * $goods['commission_sum'] / 100, 2) : 0;
				}
			}
			return $commission;
		}
		...
		public function createMyShopQrcode($mid = 0, $posterid = 0) {
			...
			//TODO 没有个人小店了  &c=entry&do=shop&m=ewei_shop
			//$url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&m=ewei_shop&do=plugin&p=commission&method=myshop&mid=' . $mid;
			$url = $_W['siteroot'] . 'app/index.php?i=' . $_W['uniacid'] . '&c=entry&do=shop&m=ewei_shop&mid=' . $mid;
			if (!empty($posterid)) {
		...
		public function createShopImage($shop_set) {
			...
			if (!is_file($path . $file)) {
				...
				$yc = imagecolorallocate($target, 255, 255, 0);
				//115 130  16
				$gc = imagecolorallocate($target, 115, 130, 16);
				$bg = imagecreatefromjpeg($shop_set['signimg']);
				imagecopy($target, $bg, 0, 0, 0, 0, 640, 1225);
				imagedestroy($bg);
				$avatar = preg_replace('/\/0$/i', '/96', $userinfo['avatar']);
				...
				$h = imagesy($head);
				$offsetX=88;
				imagecopyresized($target, $head, 44+$offsetX, 32, 0, 0, 108, 108, $w, $h);
				imagedestroy($head);
				//$thumb = $this -> createImage($shop_set['signimg']);
				//$thumb_w = imagesx($thumb);
				//$thumb_h = imagesy($thumb);
				//imagecopyresized($target, $thumb, 0, 160, 0, 0, 640, 640, $thumb_w, $thumb_h);
				//imagedestroy($thumb);
				...
				imagecopyresized($target, $qrcode, 320-ceil(210/2), 545, 0, 0, 210, 210, $qrcode_w, $qrcode_h);
				imagedestroy($qrcode);
				$str1 = '我是';
				imagettftext($target, 20, 0, 170+$offsetX, 70, $bc, $font, $str1);
				imagettftext($target, 20, 0, 230+$offsetX, 70, $cc, $font, $userinfo['nickname']);
				$str2 = '我要为';
				imagettftext($target, 20, 0, 170+$offsetX, 105, $bc, $font, $str2);
				$str3 = $shop_set['name'];
				imagettftext($target, 20, 0, 230+$offsetX+30, 105, $gc, $font, $str3);
				$box = imagettfbbox(20, 0, $font, $str3);
				$width = $box[4] - $box[6];
				$str4 = '代言';
				imagettftext($target, 20, 0, 260 + $width+10+$offsetX, 105, $bc, $font, $str4);
				$str5 = '长按二维码立刻加入';
				imagettftext($target, 20, 0, 310-ceil(210/2), 800, $gc, $font, $str5);
				imagejpeg($target, $path . $file);
				imagedestroy($target);
			}
			return $_W['siteroot'] . "addons/ewei_shop/data/poster/" . $_W['uniacid'] . "/" . $file;
		...
		public function createShopImage_back_up($shop_set) {
			global $_W, $_GPC;
			$shop_set = set_medias($shop_set, 'signimg');
			$path = IA_ROOT . "/addons/ewei_shop/data/poster/" . $_W['uniacid'] . '/';
			if (!is_dir($path)) {
			...
			if (!is_file($path . $file)) {
				...
				$qrcode_w = imagesx($qrcode);
				$qrcode_h = imagesy($qrcode);
				imagecopyresized($target, $qrcode, 50, 845, 0, 0, 210, 210, $qrcode_w, $qrcode_h);
				imagedestroy($qrcode);
				$str1 = '我是';
				imagettftext($target, 20, 0, 150, 70, $bc, $font, $str1);
				imagettftext($target, 20, 0, 210, 70, $cc, $font, $userinfo['nickname']);
		...
		function getShop($m) {
			...
			if (empty($shop)) {
				//TODO 不能有个人小店 $member['nickname'] . '的小店'
				$mid=0;
				if($member['status']==1 && $member['isagent']==1){
					$mid=$member['id'];
				}
				$shop = array('name' => "酵果菜谱" ,'mid' => $mid , 'logo' => $member['avatar'], 'img' => tomedia($set['img']), 'desc' => empty($set['description']) ? $set['name'] : $set['description'], );
			} else {
				if (empty($shop['name'])) {
					$shop['name'] = "酵果菜谱"; //$member['nickname'] . '的小店';
				}
				if (empty($shop['logo'])) {
		...
		function sendMessage($openid = '', $data = array(), $message_type = '') {
			...
			if ($message_type == TM_COMMISSION_AGENT_NEW && !empty($tm['commission_agent_new']) && empty($usernotice['commission_agent_new'])) {
				...
				$message = str_replace('[旧一级推广比例]', $data['oldlevel']['commission1'] . '%', $message);
				$message = str_replace('[旧二级推广比例]', $data['oldlevel']['commission2'] . '%', $message);
				$message = str_replace('[旧三级推广比例]', $data['oldlevel']['commission3'] . '%', $message);
				$message = str_replace('[新等级]', $data['newlevel']['levelname'], $message);
				$message = str_replace('[新一级推广比例]', $data['newlevel']['commission1'] . '%', $message);
				$message = str_replace('[新二级推广比例]', $data['newlevel']['commission2'] . '%', $message);
				$message = str_replace('[新三级推广比例]', $data['newlevel']['commission3'] . '%', $message);
				$msg = array('keyword1' => array('value' => !empty($tm['commission_upgradetitle']) ? $tm['commission_upgradetitle'] : '推广等级升级通知', "color" => "#73a68d"), 'keyword2' => array('value' => $message, "color" => "#73a68d"));
				if (!empty($templateid)) {
					m('message') -> sendTplNotice($openid, $templateid, $msg);
				} else {
					m('message') -> sendCustomNotice($openid, $msg);
				...
				$msg = array('keyword1' => array('value' => !empty($tm['commission_becometitle']) ? $tm['commission_becometitle'] : '成为推广商通知', "color" => "#73a68d"), 'keyword2' => array('value' => $message, "color" => "#73a68d"));
				if (!empty($templateid)) {
					m('message') -> sendTplNotice($openid, $templateid, $msg);
				} else {
					m('message') -> sendCustomNotice($openid, $msg);
		...
		function perms() {
			return array('commission' => array('text' => $this -> getName(), 'isplugin' => true, 'child' => array('cover' => array('text' => '入口设置'), 'agent' => array('text' => '推广商', 'view' => '浏览', 'check' => '审核-log', 'edit' => '修改-log', 'delete' => '删除-log', 'user' => '查看下线', 'order' => '查看推广订单(还需有订单权限)'), 'level' => array('text' => '推广商等级', 'view' => '浏览', 'add' => '添加-log', 'edit' => '修改-log', 'delete' => '删除-log'), 'apply' => array('text' => '佣金审核', 'view1' => '浏览待审核', 'view2' => '浏览已审核', 'view3' => '浏览已打款', 'view_1' => '浏览无效', 'check' => '审核-log', 'pay' => '打款-log', 'cancel' => '重新审核-log'), 'notice' => array('text' => '通知设置-log'), 'set' => array('text' => '基础设置-log'))));
		}
