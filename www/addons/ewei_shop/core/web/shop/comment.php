...
if ($operation == 'display') {
	...
	list($age,$gender,$career)=explode('_', $item['ext']);
	$item['age']=$age;
	$item['gender']=$gender;
	$item['career']=$career;
	...
	if (checksubmit()) {
		...
		$data['ext']="{$_GPC['age']}_{$_GPC['gender']}_{$_GPC['career']}";
		if (!empty($id)) {
	...
	if (checksubmit()) {
		$data = array('uniacid' => $_W['uniacid'],'status' =>isset($_GPC['status'])?intval($_GPC['status']):0, 'reply_content' => $_GPC['reply_content'], 'reply_images' => is_array($_GPC['reply_images'])?iserializer($_GPC['reply_images']):iserializer(array()), 'append_reply_content' => $_GPC['append_reply_content'], 'append_reply_images' => is_array($_GPC['append_reply_images'])?iserializer($_GPC['append_reply_images']):iserializer(array()),);
		$data['ext']="{$_GPC['age']}_{$_GPC['gender']}_{$_GPC['career']}";		 
		pdo_update('ewei_shop_order_comment', $data, array('id' => $id));
	...
	$cid=intval($_GPC['cid']);
	$status=intval($_GPC['check']);
	$result=pdo_update('ewei_shop_order_comment',array('status'=>$status),array('id'=>$cid));
	if($result){
		die(json_encode(array('status'=>200)));
	}
	die(json_encode(array('status'=>201,'msg'=>'系统异常，请稍候再试')));
} 
load() -> func('tpl');
