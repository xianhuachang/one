...
class Ewei_DShop_Shop {
	...
	public function getSecondCategory() {
		...
		//$shopset = m('common') -> getSysset('shop');
		$allcategory = array();
		$category = pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_category') . " WHERE uniacid=:uniacid and enabled=1 and parentid<>0 ORDER BY parentid ASC, displayorder DESC", array(':uniacid' => $_W['uniacid']));
		/*$category = set_medias($category, array('thumb', 'advimg'));
		foreach($category as $c) {
			if (empty($c['parentid'])) {
				$children = array();
