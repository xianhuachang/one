...
if (!class_exists('PosterModel')) {
	class PosterModel extends PluginModel {
		...
		public function createCommissionPoster($openid, $goodsid = 0) {
			...
			$sql='select * from ' . tablename('ewei_shop_poster') . ' where uniacid=:uniacid and type=:type and isdefault=1 limit 1'; 			
			$poster = pdo_fetch($sql, array(':uniacid' => $_W['uniacid'], ':type' => $type));
			if (empty($poster)) {
				return '';
			} 
