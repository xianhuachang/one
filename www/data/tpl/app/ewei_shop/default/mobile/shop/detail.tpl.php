...
.good_shops {height:59px; width:94%; background:#fff; padding:0px 3%; margin-top:14px; border-bottom:1px solid #e2e2e2; border-top:1px solid #e2e2e2;}
.good_shops .shops2 {height:59px; width:100%; padding:10px 0px;}
.good_shops .shops2 .subs1 {height:37px; width:47%; float:left; border:1px solid #ccc; border-radius:5px; text-align:center; line-height:37px; font-size:16px; color:#999;}
.good_shops .shops2 .subs2 {height:37px; width:47%; float:right; border:1px solid #ccc; border-radius:5px; text-align:center; line-height:37px; font-size:16px; color:#999;}
...
.good_info2 .menu .nav {height:40px; width:33%; border-right:1px solid #e3e3e3; float:left; font-size:14px; color:#666; text-align:center; line-height:40px; border-bottom:1px solid #e3e3e3;}
...
.good_info2 .tab_con {height:auto; width:99%; padding: 10px 0 10px 1%; padding-bottom:30px;}
...
.copyright {height:14px; font-size:14px; line-height:14px; padding:10px 0px; margin-bottom:50px; text-align:center; color:#aaa;}
...
.good_bottom .like,.good_bottom .report {height:42px; width:14%; background:#fdfdfd; float:left; padding-top:7px; border-top:1px solid #e1e1e1; border-right:1px solid #e1e1e1; text-align:center; font-size:20px; color:#666; line-height:10px;}
...
div.flicking_con a.on{background-position:0 -9px}
   ...
   .comment { padding:5px; border-bottom:1px solid #ccc;overflow:hidden;}
    ...
    #recommand_container .good {height:auto;  width:28%; background:#fff; padding:1%; margin:5px 1%; float:left;border:1px solid rgb(211, 215, 212);}
    #recommand_container .good .img {width:100%;overflow:hidden;}
    #recommand_container .good .img img {width:100%;height:120px;}
    #recommand_container .good .name {height:50px; width:100%; font-size:14px; color:#999; line-height:25px;overflow:hidden}
    #recommand_container .good .price {height:25px; width:100%; font-size:16px; color:#feabb4;}
    #recommand_container .good .price span {height:25px; width:auto;font-size:16px;padding:2px 6px; background:#F5721A; color:#fff; font-size:12px; border-radius:5px;}
    #recommand_container .good .price i {height:25px;font-size:14px; width:auto; padding:2px 6px; background:#666; color:#fff; font-size:12px; border-radius:5px; text-decoration: line-through}
...
.red{color: red;}
#comment_switch{text-align: center;letter-spacing: 5px;border: 1px solid red;margin: 10px auto;padding: 5px 0px;border-radius: 10px;}
...
<script id='goods_info'  type='text/html'>
    ...
    <div class="good_choose">
        <div class="info">
             ...
             <div class="right">
                   ...
                   <div class="stock">库存:<span id='option_stock'><%goods.sale_quotas%></span>件</span> </div>
                    <%if goods.hasoption!='0'%>
                   <div class="option">请选择规格(目前只推出鲜果型)</div>
                   <%/if%>
             </div>
        ...
        <div class="other">
            <input type='hidden' id='optionid' value='' />
                ...
                <div class='number' style="display:none"> 
                  ...
                  <div class='num'>
                            <button id='btn_minus' disabled  onclick='reduceNum()'><i class='fa fa-minus'></i></button>
                            <input type='text' id='total' readonly value='1' />
                            <button id='btn_plus' disabled onclick='addNum()'><i class='fa fa-plus'></i></button>
                  </div>
    ...
    <div class="good_info1">
        ...
        <div class="price">￥<d id='marketprice'>
                ...
                <%goods.minprice%>/份/450ml <!-- - <%goods.maxprice%>-->
                <%else%>
                <%goods.marketprice%>
                <%/if%>
				(下单有优惠)
            ...
            <span id='productpricecontainer'>
                市场价￥<span id='productprice'><%goods.productprice%></span></span>
        </div>
       
        <div class="other">
        	<div class="left">
        	<!--<span class="red">* 此产品不支持7天退货</span>-->
        	</div>
            <div class='right'>
                 <span id="stockcontainer">库存:<span id="stock"><%goods.sale_quotas%></span></span> 销量:<%goods.sales%><%goods.unit%>
				 </div>
            </div>
     ...
     <%/each%> 
      <%each stores as store index%>
     <%if index>1%>
     <div class="store" id="store_more" style="display:none">
             <div class="info">
             <div class="ico"><i class="fa fa-building-o"></i></div>
             <div class='info1'>
                 <div class='inner'>
    ...
    <%if stores.length>=3%>
     ...
     <%/if%>
    <div class="good_speci" <%if goods.canbuy && goods.total!=0 && goods.timebuy=='0' && goods.userbuy=='1'  && goods.groupbuy=='1'  && goods.levelbuy=='1'%> <%/if%>><!--onClick="choose()"--> 
        <%if goods.canbuy%>
            <%if goods.istime=='1'%>
              ...
              <%else%>
                 ...
                 <%else%>
                       <%if goods.groupbuy=='0' || goods.levelbuy=='0'%>
                        ...
                        <%else%>
                            <%if goods.total!=0%>
                            <span id="optiondiv">提示：下单前请选择商品规格及数量</span>
                             <%else%>
                              库存不足，无法购买
                           <%/if%>
                        <%/if%>
      ...
      <!-- <i class="fa fa-angle-right" style="float:right; line-height:44px; font-size:26px;"></i>-->
        ...
        <div class="good_shops">
        <div class="shops2">
            <div class="subs1" onclick="location.href='<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%goods.Backid%>'">看上一个商品</div>
            <div class="subs2" onclick="location.href='<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%goods.Frontid%>'"> 看下一个商品</div>
        </div>
    </div>


    <div class="good_shop">
        <div class="shop1" onclick="location.href='<%shop.url%>'">
        <img src="<%shop.logo%>">
        <div class="shop_info">
        <%shop.name%><br><span>全部宝贝<%goodscount%>个</span></div></div>
        <div class="shop2">
    ...
    <div class="good_info2">
        <div class="menu">
            ...
            <div id="nav_3" class="nav"  style="display: none;"  ></div>
            <div id="nav_4" class="nav" onClick="tab(4)" style="border-right:0px;">同店推荐</div>
        </div>
        <div class="tab_con">
            <div class="con" id="con_1"  style='display:block'>
                <%=goods.content%>
                <hr/>
                <p style="margin: 0px">案例分享:</p>
                <div class="con" id="con_3" style='display:block'>
	                <div id='comment_container'>
	                	<div id="comment_switch" style="width:80%">加载案例</div>
	                </div>
            	</div>
            </div>
          ...
          <!--<div class="con" id="con_3" ><div id='comment_container' style="display:none"></div></div>-->
    ...
    <div class="good_bottom">
       ...
       <%if uncomfortable_place!=''%>
        	<div class="report"><i class="fa fa-search"></i><br><span>报告</span></div>
        <%/if%>
        <%if goods.isverify!='2'%>
        ...
        <div class="buy" onclick="choose('buy')" <%if goods.isverify=='2'%> style='width:80%'<%/if%>>立即购买</div>
</div>
    <%/if%>
</script>
<script id='tpl_comment' type='text/html'>
    ...
    <div class='comment' style="display:none">
                    <div class='info'>
                        ...
                        <div class='nickname'><div class='inner'><%comment.nickname%> <%comment.gender%> <%comment.age%> <%comment.career%></div></div>
                        <div class='level'>
...
<script id='tpl_recommand' type='text/html'>
    ...
    <div class="good" data-goodsid='<%g.id%>'>
        ...
        <div class="price"><span>￥<%g.marketprice%></span> <%if g.productprice>0 && g.marketprice!=g.productprice%><%/if%></div>
    </div>
    <%/each%>
</script>
    ...
    require(['tpl', 'core','jquery.touchslider','swipe'], function(tpl, core) {
        ...
        $(window).scroll(function(){
         ...
         $('.good_img').height( document.body.clientWidth + "px");
          ...
          $('#con_1 img,#con_1 table,#con_1 div').not("#comment_switch").width('100%');
         ...
         if(thumb!=''){
          ...
          function bindCommentImages(){
                    ...
                    page = 1;
                     core.json('shop/util',{op:'comment', page:page,goodsid:'<?php  echo $_GPC['id'];?>'},function(json){
                         if(json.result.list.length<=0){
                             $('#comment_container').html('暂时没有任何案例分享').css("display","block");
                             return;
                         }
                         $('#comment_container').append( tpl('tpl_comment',json.result));
                       bindCommentImages();
                            var loaded = false;
                              var stop=true; 
                              $(window).scroll(function(){ 
                                  if(loaded){
                                    ...
                                    if($(document).height() <= totalheight){ 
                                        ...
                                        if(stop==true){ 
                                            stop=false; 
                                            $('#comment_container').append('<div id="comment_loading" style="display: none;"><i class="fa fa-spinner fa-spin"></i> 正在加载...</div>');
                                            page++;
                                            core.json('shop/util', {op:'comment',page:page,goodsid:'<?php  echo $_GPC['id'];?>'}, function(morejson) {  
                                                ...
                                                if (morejson.result.list.length <morejson.result.pagesize) {
                                                    ...
                                                    $("#comment_container").append('<div id="comment_loading" style="display: none;">已经加载完全部评价</div>');
                                                    loaded = true;
                                                    $(window).scroll = null;
                                                    return;
                                                }
           ...
           $("#comment_switch").click(function(){
	      		...
	      		$("#comment_container .comment").css("display","block");
	      		$("#recommand_container").css("display","block");
	      	});
...
<script type="text/javascript">
	function tab(n){
        if(n!=3) {
        	console.log('#con_' + n);console.log('#nav_' + n);
            $('#con_' + n).fadeIn();
...
<?php  if($uncomfortable_place!="") { ?>
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close red" style="float: right;border: none;" data-dismiss="modal" aria-label="Close">
					<span aria-hidden="true">&times;</span>
				</button>
				<h4 class="modal-title" id="myModalLabel">我的自诊报告</h4>
			</div>
			<div class="modal-body">
				<div class="content">
					<?php  if(is_array($uncomfortable_place)) { foreach($uncomfortable_place as $item) { ?>
					<div class="panel panel-danger">
					  <div class="panel-heading">身体部位:<?php  echo $item['name'];?></div>
					  <div class="panel-body">
					   <?php  if(is_array($item['questions'])) { foreach($item['questions'] as $q_item) { ?>
						<p class="top red"><?php  echo $q_item['name'];?></p>
						<p><?php $resultArr=explode(';',$q_item['solution']); echo empty($q_item['detail']) ?$resultArr[1] : $resultArr[0]; ?></p>
						<?php  } } ?>
					  </div>
					</div>
					<?php  } } ?>
					<div class="product-recommend">
						<div class="panel panel-success">
						  ...
						  <div class="panel-body">
						  	<?php  if(is_array($recommend_products)) { foreach($recommend_products as $item) { ?>
...
<style>
   .good_bottom .add,.good_bottom .buy {width:28%}
	.product-img{width: 80px;}
    .product-item{float: left; width: 90px;margin-left: 5px;text-align: center;}
</style>
<link rel="stylesheet" type="text/css" href="../addons/ewei_shop/static/modal/bootstrap.css">
<script language="javascript" src="../addons/ewei_shop/static/modal/bootstrap.min.js"></script>
<?php  } ?>
<?php  $hide_footer=1?>
